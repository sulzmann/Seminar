...
Aktionspunkte und Fragen.

*QuickCheck-Haskell.*

* Definiere eine `arbitrary` Instanz für `data List a = Nil | Cons a (List a)`

* Betrachte eigene Beispiele (z.B. ein Testfall aus einer Vorlesung den wir versuchen in QuickCheck-Haskell umzusetzen)

*QuickCheck-C++.*

* Betrachte bestehende QuickCheck Umsetzungen in C++

* Ziehe einen Vergleich mit Quickcheck in C++ (OO Ansatz) und Quickcheck in C++ (FP Ansatz)

* Betrachte bestehende QuickCheck Umsetzungen in anderen OO Programmiersprachen

*C++ templates*.

* Die Quickcheck in C++ (FP Ansatz) verwendet "(function) template instantiation"

* Finde dazu entsprechende Referenzen und weitere Beispiele
...
* Diskutiere Rust Typfehlermeldungen (wegen Speicherfehler) und betrachte Tools welche dem User helfen diese Fehler zu beseitigen

*Rust enums und traits.*

* Betrachte einen enum Datentyp, z.B. kodiere obigen `List` Datentyp

* Erkläre das Konzept von "pattern matching" welches erlaubt die verschiedenen Varianten eines enums zu analysieren

* Betrachte traits anhand einfacher Beispiele
