% Seminar Winter 24/25: Fuzz Testing
% Martin Sulzmann
...
2. Start a github repo (add description, examples, report, ...)

3. Attend our meetings and participate in discussions

4. After each meeting we collect a list of action items

5. Give 1-2 talks where you report about the progress you have made
...
which may lead to further inprecision of the analysis.
...
* Strings must satify certain patterns (e.g. expressible in terms of regular expressions).

* How to effetively generate inputs in such a case?
...
# Fuzzing in Go by example (summary)
...
In case of the channel example it took the fuzzer a long time (10secs+) till the Go fuzzer reports "hung".
What if we consider concurrent programs where there are many more alternative schedules?


# Topics

Here comes a list of seminar topics.
Each topic is labled as <b>Ti</b>.
...
1. Build your own "larger" Go application. For example, you could (re)program some of the Softwareproject exercises in Go.
...
Besides Go's builin fuzzer there exists several reserach tools
